...
func init() {
	Register(HTTPRouteCanaryHeaderGateway)
}

var HTTPRouteCanaryHeaderGateway = suite.ConformanceTest{
	ShortName:   "HTTPRouteCanaryHeaderGateway",
	Description: "The Ingress translate to Gateway in the higress-conformance-infra namespace uses the canary header traffic split.",
	Features:    []suite.SupportedFeature{suite.Ingress2GatewayConformanceFeature},
	Test: func(t *testing.T, suite *suite.ConformanceTestSuite) {
		// read ingress file, convert to gateway resource and apply to cluster
		ToGatewayResource(t, suite, "tests/httproute-canary-header.yaml")
		...
		testcases := []http.Assertion{
			...
			// todo Higress does not support RegularExpression yet
			// {
			// 	Meta: http.AssertionMeta{
		...
		t.Run("Gatewayapi Canary HTTPRoute Traffic Split", func(t *testing.T) {
